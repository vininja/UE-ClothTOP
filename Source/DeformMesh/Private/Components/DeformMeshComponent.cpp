...
public:
	...
	*/
	static bool ShouldCompilePermutation(const FVertexFactoryShaderPermutationParameters& Parameters)
	{
		if ((Parameters.MaterialParameters.MaterialDomain == MD_Surface &&
			(Parameters.MaterialParameters.ShadingModels == MSM_Unlit ||
				...
				Parameters.MaterialParameters.ShadingModels == MSM_Subsurface)) ||
	...
	/* This is the equivalent to manually setting preprocessor directives, so when compilation happens, only the code that we're interested in gets in the compiled shader*/
	...
	virtual void InitRHI() override 
	{
		...
		FVertexDeclarationElementList PosAndNormOnlyElements; // Used for the PositionOnly vertex stream
		...
		if (Data.PositionComponent.VertexBuffer != NULL)
		{
			...
			//PosAndNormOnlyElements.Add(AccessStreamComponent(Data.PositionComponent, 0, EVertexInputStreamType::PositionAndNormalOnly));
		}
		
		//InitDeclaration(PosOnlyElements, EVertexInputStreamType::PositionOnly);
		//InitDeclaration(PosAndNormOnlyElements, EVertexInputStreamType::PositionAndNormalOnly);
		...
		if (Data.TextureCoordinates.Num())
		{
			...
			for (int32 CoordinateIndex = 0; CoordinateIndex < Data.TextureCoordinates.Num(); CoordinateIndex++)
			{
				...
				PosOnlyElements.Add(AccessStreamComponent(
					Data.TextureCoordinates[CoordinateIndex],
					BaseTexCoordAttribute + CoordinateIndex
				));
				
				PosAndNormOnlyElements.Add(AccessStreamComponent(
					Data.TextureCoordinates[CoordinateIndex],
					BaseTexCoordAttribute + CoordinateIndex
				));
			}
		...
		InitDeclaration(PosOnlyElements, EVertexInputStreamType::PositionOnly);//Initialize the Position Only vertex declaration which will be used in the depth pass
		InitDeclaration(PosAndNormOnlyElements, EVertexInputStreamType::PositionAndNormalOnly);
	...
	inline void SetTouchEngineComp(UTouchEngineComponentBase* TEC) { TouchEngineComponent = TEC; }
private:
...
public:
		...
		, MaterialRelevance(Component->GetMaterialRelevance(GetScene().GetFeatureLevel()))
	{
		...
		for (uint16 SectionIdx = 0; SectionIdx < NumSections; SectionIdx++)
		{
			const FDeformMeshSection& SrcSection = Component->DeformMeshSections[SectionIdx];
			{
				...
				VertexFactory->SetTouchEngineComp(SrcSection.TouchEngineComponent);
	...
	void UpdateDeformTransformsSB_RenderThread()
	{
		...
		if(bDeformTransformsDirty && DeformTransformsSB.IsValid())
		{
...
public:
	void Bind(const FShaderParameterMap& ParameterMap)
	{
		...
		//new
		PositionTextureParameter.Bind(ParameterMap, TEXT("posTEX"), SPF_Optional);
		NormalTextureParameter.Bind(ParameterMap, TEXT("normTEX"), SPF_Optional);
		PNSamplerParameter.Bind(ParameterMap, TEXT("PNSampler"), SPF_Optional);
	};
	...
	) const
	{
		...
		if (DeformMeshVertexFactory->TouchEngineComponent->EngineInfo->GetTOPOutput("op/posTOP").Texture != nullptr)
		{
			FTextureResource* PosTextureResource = DeformMeshVertexFactory->TouchEngineComponent->EngineInfo->GetTOPOutput("op/posTOP").Texture->Resource;
			ShaderBindings.AddTexture(PositionTextureParameter, PNSamplerParameter, PosTextureResource->SamplerStateRHI, PosTextureResource->GetTexture2DRHI());
		}

		if (DeformMeshVertexFactory->TouchEngineComponent->EngineInfo->GetTOPOutput("op/normTOP").Texture != nullptr)
		{
			FTextureResource* NormTextureResource = DeformMeshVertexFactory->TouchEngineComponent->EngineInfo->GetTOPOutput("op/normTOP").Texture->Resource;
			ShaderBindings.AddTexture(NormalTextureParameter, PNSamplerParameter, NormTextureResource->SamplerStateRHI, NormTextureResource->GetTexture2DRHI());
		}
...
private:
	...
	LAYOUT_FIELD(FShaderResourceParameter, PositionTextureParameter);
	LAYOUT_FIELD(FShaderResourceParameter, NormalTextureParameter);
	LAYOUT_FIELD(FShaderResourceParameter, PNSamplerParameter);
};
...
///////////////////////////////////////////////////////////////////////
/*
 * Most of this method below are self explanatory, they make changes to the game thread state and propagate changes to the render thread using the scene proxy
*/
void UDeformMeshComponent::CreateMeshSection(int32 SectionIndex, UStaticMesh* ClothMesh, UTouchEngineComponentBase* TouchEngineComponent, const FTransform& DeformTransform)
{
	...
	NewSection.StaticMesh = ClothMesh;
	NewSection.DeformTransform = DeformTransform.ToMatrixWithScale().GetTransposed();
